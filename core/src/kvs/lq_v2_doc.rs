...
mod test {
	...
	fn test_construct_document_update() {
		...
		let current_value = Value::Object(Object(map! {
			"first_field".to_string() => Value::String("first_value".to_owned()),
			"second_field".to_string() => Value::String("second_value".to_owned()),
		}));
		let operations = vec![
			Operation::Remove {
			...
			Operation::Replace {
				...
				value: Value::String("original_value".to_owned()),
			},
			Operation::Add {
				path: Idiom::from("third_field"),
				value: Value::String("third_value".to_owned()),
			},
		];
		let expected_original = Value::Object(Object(map! {
			"second_field".to_string() => Value::String("original_value".to_owned()),
			"third_field".to_string() => Value::String("third_value".to_owned()),
		}));
		let tb_mutation =
	...
	fn test_construct_document_delete_with_original() {
		...
		let original = Value::Object(Object(map! {
			...
			"some_key".to_string() => Value::String("some_value".to_owned()),
		}));
...
mod test_check_lqs_and_send_notifications {
	...
	fn a_live_query_statement() -> LiveStatement {
		...
		session.insert(OBJ_PATH_ACCESS.to_string(), Value::String("access".to_owned()));
		session.insert(OBJ_PATH_AUTH.to_string(), Value::String("auth".to_owned()));
		session.insert(OBJ_PATH_TOKEN.to_string(), Value::String("token".to_owned()));
