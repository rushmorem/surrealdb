...
impl Parser<'_> {
	...
	async fn parse_object_or_geometry_after_type(
		...
		match type_value.as_str() {
				...
				if !self.eat(t!(",")) {
					...
					return self
						.parse_object_from_map(
							ctx,
							BTreeMap::from([(key, Value::String(type_value))]),
							start,
						)
						.await
						.map(Value::Object);
			...
			_ => {
				let object = BTreeMap::from([(key, Value::String(type_value))]);
	...
	async fn parse_object_or_geometry_after_coordinates(
		...
		if !ate_comma {
			...
			return Ok(Value::Object(Object(BTreeMap::from([
				(key, value),
				(type_key, Value::String(type_value)),
			]))));
		}

		self.parse_object_from_map(
			ctx,
			BTreeMap::from([(key, value), (type_key, Value::String(type_value))]),
			start,
		)
		.await
		.map(Value::Object)
		...
		Fm: FnOnce(R) -> Value,
	{
		if !self.eat(t!(",")) {
			...
			self.expect_closing_delimiter(t!("}"), start)?;
			return Ok(Value::Object(Object(BTreeMap::from([(key, Value::String(strand))]))));
		}
		let coord_key = self.parse_object_key()?;
		if coord_key != "coordinates" {
			...
			return self
				.parse_object_from_key(
					ctx,
					coord_key,
					BTreeMap::from([(key, Value::String(strand))]),
					start,
				)
				.await
				.map(Value::Object);
		...
		if !self.eat(t!("}")) {
			...
			return self
				.parse_object_from_map(
					ctx,
					BTreeMap::from([(key, Value::String(strand)), (coord_key, value)]),
					start,
				)
				.await
				.map(Value::Object);
